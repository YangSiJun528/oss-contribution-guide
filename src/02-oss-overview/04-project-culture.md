...
`README`, `CONTRIBUTING`, `CODE_OF_CONDUCT`, `SECURITY`, Issue·PR 템플릿을 먼저 읽고 최근 Issue와 기여 사례를 함께 확인합니다. Discord나 Slack에서 나눈 대화가 있더라도 최종 결정이 Issue, 메일링 리스트, RFC 같은 공식 기록에 남는 프로젝트라면 그 기록을 기준으로 삼아야 합니다.
