...
『오픈 소스로 미래를 연마하라』는 역할을 사용자, 신입 기여자, 일반 기여자, 핵심 기여자와 리더십으로 이어지는 양파 모형으로 설명합니다. 중심에 가까울수록 활동과 책임이 커지지만 중요도의 서열은 아닙니다.
...
> “한 사람이 여러 가지 역할을 맡은 걸 볼 때도 있지만(특히 작은 프로젝트에서) 그렇지 않은 프로젝트에서는 여러 사람이 한 가지씩 역할을 맡아 책임을 나눈다.”
>
> — VM 브라수어, 『오픈 소스로 미래를 연마하라』, 40쪽

사용자는 프로젝트가 존재하는 이유이자 문제와 요구사항을 가장 먼저 발견하는 사람일 수 있습니다. 따라서 바깥 역할이 중심의 역할보다 덜 중요하다는 뜻은 아닙니다. 모든 사람이 중심으로 이동해야 하는 것도 아니며, 원하는 범위에서만 참여할 수 있습니다.

신입 기여자를 위한 절차와 도움을 갖춘 프로젝트는 처음 참여하는 사람이 정착하기 쉽습니다. 프로젝트를 고를 때 첫 기여 문서, 초보자용 Issue와 질문 창구가 있는지도 확인할 수 있습니다.
