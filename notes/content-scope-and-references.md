...
> 공개 본문이 아니라 집필할 때 참고하는 내부 문서다. 현재 목차를 바로 줄이지 않고, 어떤 내용을 직접 쓰고 어떤 내용을 외부 자료로 넘길지 정리한다.
...
| 주제 | 응답 수 | 처리 |
|---|---:|---|
| 기여 문화와 태도 | 6 | 2.5와 3장에서 직접 설명 |
| 프로젝트 선택 기준 | 5 | 4.1에서 직접 설명 |
| 첫 이슈·작업 선택 | 4 | 4.3에서 직접 설명 |
| AI 기준과 주의점 | 3 | 5장에서 직접 설명하고 정책 사례 연결 |
| 대규모 코드베이스 파악 | 2 | 4.4에 짧은 체크리스트 제공 |
| 장기적인 기여 | 2 | 6장에서 선택지로 제시 |
| 프로젝트별 절차 | 2 | 4.2에 확인 목록 제공 |
| 코드 수정과 테스트 | 1 | 4.5에 검증 기준 제공 |
...
| 범위 | 처리 | 넘길 자료 |
|---|---|---|
| 1.1 이 가이드가 다루는 것 | 직접 | 없음. 코드 기여의 범위와 `검토 가능한 PR 제출`은 이 가이드의 기준이다. |
...
| 2.4 관리·소통 방식 | 합치기 | “GitHub 외의 방식도 있다”만 2.5에 남긴다. |
| 2.5 프로젝트 문화 | 직접 | 최근 Issue·PR과 공식 채널에서 실제 규칙을 확인하는 방법을 다룬다. |
| 3.1–3.5 기여자의 태도 | 한 글로 합치기 | 제안, 검토·유지 비용, 관계, 프로젝트 존중을 하나의 흐름으로 설명한다. AI 세부 기준은 5장으로 보낸다. |
...
| 5.1–5.5 AI와 OSS | 두 글로 합치기 | `AI가 만든 변화와 사례`, `AI를 사용할 때의 원칙`으로 압축한다. 정책 예시는 아래 자료를 사용한다. |
| 6.1 PR 결과 | 직접 | 병합 여부와 이 가이드의 목표 달성을 분리해 설명한다. |
| 6.2 다음 참여 | 합치기 | 6장 첫 페이지나 6.1의 끝에 선택지로 넣는다. |
| 6.3 경험 정리 | 직접 | 모임에서 공유할 짧은 회고 양식을 둔다. |
| 7.1 기부와 인증 | 직접 | 일반적인 OSS 규칙이 아니라 모임 참가 규칙임을 명시한다. |
| 7.2 이 가이드 개선 | 링크 중심 | 실제 `CONTRIBUTING.md`가 생기면 그 파일로 보낸다. |
| 7.3 추천 자료 | 링크 중심 | 독자가 사용할 핵심 자료와 한 줄 설명만 둔다. |
| 7.4 라이선스 | 직접 | 이 저장소의 콘텐츠·예제 코드·제3자 자료 구분만 명시한다. |

지금 34개 글에서 합칠 항목을 반영하면 약 25개가 남는다. 2장과 5장의 요약 글 일부를 섹션 첫 페이지로 옮기면 약 18~20개까지 줄일 수 있다. 우선순위는 3장과 5장을 먼저 합치고, 4장은 현재 흐름을 유지하는 것이다.
